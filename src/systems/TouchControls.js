...
export class TouchControls {
  ...
  _buildUI() {
    ...
    const heavyBtn = this._makeButton(w - 105, h - 215, BTN_RADIUS, 0xff5a5a, 'K');
    const lightBtn = this._makeButton(w - 215, h - 125, BTN_RADIUS, 0xffcf5a, 'J');
    const dodgeBtn = this._makeButton(w - 95, h - 95, BTN_RADIUS * 0.8, 0x5ad1ff, 'DODGE');
    ...
    const qBtn = this._makeButton(w - 330, h - 330, abilRadius, 0x9a5aff, 'Q');
    const eBtn = this._makeButton(w - 220, h - 330, abilRadius, 0x9a5aff, 'E');
    const rBtn = this._makeButton(w - 110, h - 330, abilRadius, 0x9a5aff, 'R');
  ...
  _makeButton(x, y, radius, color, label) {
    ...
    const text = scene.add.text(x, y, label, {
      fontFamily: 'Silkscreen', fontSize: label.length > 1 ? '15px' : '24px', color: '#fff',
