...
await page.addInitScript(() => localStorage.setItem('akhand_quality', 'low'));
...
async function go(ri) {
  ...
  await page.waitForFunction(() => window.__game?.scene?.isActive('GameScene'), { timeout: 90000 });
  ...
  await page.screenshot({ path: path.join(OUT, `region_${ri}_shot.png`), timeout: 90000 });
}
